...
export async function POST(request: Request) {
  ...
  try {
    ...
    const langAddendum = buildToolLanguageAddendum([situation, contexte].filter(Boolean).join(" "), typeof body.locale === "string" ? body.locale : undefined);
    const raw = await generateDocument(prompt, auth.billing("memoire-conformite", "memoire-conformite"), langAddendum);
